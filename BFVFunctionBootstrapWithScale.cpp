...
int main() {
    ...
    int f_zero = 51237;
    vector<uint64_t> rangeCheckIndices = fastRangeCheckIndices_63_8points_post;
    ...
    while(seal_context.last_parms_id() != scaled.parms_id()){
        evaluator.mod_switch_to_next_inplace(scaled);
    }
    ...
    for (int i = 0; i < sq_ct; i++) {
        eval_coeff.rotate_rows(scaled, sq_ct * i, gal_keys_coeff, ct_sqrt_list[i]);
