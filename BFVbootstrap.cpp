...
int main() {
    ...
    for (int gg = 0; gg < group_cnt; gg++) {
        ...
        Ciphertext coeff = slotToCoeff_WOPrepreocess(seal_context, seal_context_last, ct_sqrt_list, gal_keys_coeff, sq_ct, ring_dim, p, scalar);
        e = chrono::high_resolution_clock::now();
        ...
        // }
        // cout << endl << endl;
        // seal::util::RNSIter new_key_bfv(bfv_secret_key.data().data(), ring_dim);
        // ntt_negacyclic_harvey(new_key_bfv, coeff_modulus.size(), seal_context.key_context_data()->small_ntt_tables());
        ...
        cout << endl << endl;
        vector<regevCiphertext> lwe_ct_results = extractRLWECiphertextToLWECiphertext(coeff, ring_dim, n);
        e = chrono::high_resolution_clock::now();
        cout << "keySwitch + extraction: " << chrono::duration_cast<chrono::microseconds>(e - s).count() << endl;


        // for (int i = 0; i < (int) lwe_ct_results.size(); i++) {
        //   cout << "a:" << lwe_ct_results[i].a << endl;
        //   cout << "b:" << lwe_ct_results[i].b << endl;
        // }
        vector<int> ttmsg;
